...
def binary_accuracy(preds, labels):
    ...
    preds = torch.round(torch.sigmoid(preds))  # Converte logits para 0 ou 1
    correct = (preds == labels).float()  # Verifica se acertou
    return correct.sum().item() / labels.shape[0]  # Retorna a acurácia
...
def train_model(model, train_loader, num_epochs, learning_rate, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            total_correct_stars += binary_accuracy(stars_output, stars_labels) * stars_labels.size(0)
            total_correct_helpfulness += binary_accuracy(helpfulness_output, helpfulness_labels) * helpfulness_labels.size(0)
            total_samples += stars_labels.size(0)  
        ...
        epoch_acc_stars = total_correct_stars / total_samples * 100
        epoch_acc_helpfulness = total_correct_helpfulness / total_samples * 100
        
        print(f"Epoch [{epoch+1}/{num_epochs}], Loss: {epoch_loss:.4f}, "
              f"Stars Accuracy: {epoch_acc_stars:.2f}%, Helpfulness Accuracy: {epoch_acc_helpfulness:.2f}%")
        torch.save(model.state_dict(), "cnn_apps_1.pt")
